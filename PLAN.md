...
| **Couverture des scénarios**                          | Balayage exhaustif de 7 560 profils et de tous les profils partiels, matrice d'accès entre utilisateurs (résultat et rapport, revendiqué ou non), régimes de configuration dégradés sur toutes les routes, réservation concurrente d'un même créneau, bornes des champs libres                                                                                         | **Fait**                                           |
| **Page du rapport**                                   | Le livrable central du diagnostic n'avait aucun écran côté destinataire : rendu partagé avec la version imprimable, même contrôle d'accès que le résultat, rien avant que le rapport ne soit marqué envoyé                                                                                                                                                             | **Fait**                                           |
| **Séquence email active**                             | Passage d'envoi J+0 → J+25 branché sur le déclencheur cron, conditions d'envoi par email (le J+2 attend un rapport réellement envoyé, le J+25 une déduction réelle), borne de retard, page de désinscription et retrait de consentement effectif                                                                                                                       | **Fait**                                           |
| **Timeline interactive** | Les tâches datées ET les échéances officielles projetées sur l'axe du temps : accompli, en retard, échéance qui approche (pulsation), repère du jour, bloc « à commencer maintenant », sélection au clic reliée à l'outil de la tâche ; rendue sur la feuille de route et le tableau de bord par une projection unique ; avance mécaniquement avec les statuts | **Fait** |
| **Sélecteur d'écoles**                                | Liste de sélection alimentée par les accords détectés, classement et avancement déclarés par l'utilisateur, dates limites, constats d'équilibre sans aucune appréciation des chances d'admission, feuille de route reliée aux outils qui accomplissent les tâches                                                                                                      | **Fait**                                           |
| **Navigation mobile**                                 | Menu déroulant sous 900 px, en-tête de l'espace payant qui passe à la ligne : la navigation disparaissait purement et simplement sur téléphone                                                                                                                                                                                                                         | **Fait**                                           |
| **Phase 3 — Matrices éditables sans code**            | Règles du Moteur A (activation, source, date — la condition reste code) et blocs voie/verdict/risque éditables depuis `/admin/matrices` ; révisions append-only, blocs résolus à la date de l'évaluation, vocabulaire interdit et variables refusés à l'enregistrement                                                                                                 | **Fait**                                           |
