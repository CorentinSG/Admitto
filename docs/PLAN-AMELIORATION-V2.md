...
| F3  | Rédiger les modules 2, 4, 5, 6, 7 et 8 — **1, 3, 9 et 10 sont écrits**                                          | L'offre PLATFORM promet une bibliothèque qui existe à 45 %. Les six restants portent tous une section `OFFICIAL_RULE` : ils exigent une source officielle datée, que seul le fondateur peut confronter | `content/modules.ts` — `isPublishable` retient tout module non sourcé |
